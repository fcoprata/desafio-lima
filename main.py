...
def read_root(url):
    ...
    table_ref = client.dataset("lima_articles_raw").table("tb_articles")
...
def read_table():
    ...
    dataset_ref = client.dataset("lima_articles_raw")
...
def search(code_url: str):
    """
    A function that searches for a URL in the BigQuery table and returns the corresponding JSON data.
    ...
    query = f"SELECT * FROM `lima_articles_raw.tb_articles` WHERE URL = '{url}'"
...
def search_fonte(fonte: str):
    """
    A function that searches for articles by a specific fonte in the BigQuery table.

    Parameters:
    - fonte (str): The name of the fonte to search for.

    Returns:
    - list: A list of dictionaries containing the articles written by the fonte.
    """
    # Query the BigQuery table for articles by the fonte
    query = f"SELECT * FROM `lima_articles_raw.tb_articles` WHERE fonte = '{fonte}'"
    query_job = client.query(query)
    results = query_job.result()
...
def search_keyword(keyword: str):
    """
    A function that searches for articles containing a specific keyword in the BigQuery table.

    Parameters:
    - keyword (str): The keyword to search for.

    Returns:
    - list: A list of dictionaries containing the articles containing the keyword.
    """
    # Query the BigQuery table for articles containing the keyword
    query = f"SELECT * FROM `lima_articles_raw.tb_articles` WHERE '{keyword}' IN UNNEST(Keywords)"
    query_job = client.query(query)
    results = query_job.result()
